...
const requireSignin = expressJwt({ secret: "zohaib", userProperty: 'auth', algorithms: ['sha1', 'RS256', 'HS256'] })
...
router.post('/auth/login', async (request: express.Request, response: express.Response) => {
    ...
    try {
        ...
        if (user != null) {
            ...
            const token = jwt.sign({ _id: user._id }, "zohaib")
...
router.route('/auth/user').get(requireSignin, async (request: express.Request, response: express.Response) => {
    ...
    try {
        if (token != null) {
            const userIdFromToken = await jwt.verify(token, "zohaib") as IToken
